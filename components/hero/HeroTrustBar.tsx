      ...
      <div className="hero-trust-bar hero-trust-bar--mobile grid grid-cols-2 gap-x-2.5 gap-y-3 px-2.5 py-3 sm:grid-cols-4 sm:gap-4 sm:px-6 sm:py-5 lg:grid-cols-4">
        {items.map(({ icon: Icon, label, sublabel }) => (
          <div key={label} className="flex items-start gap-2 sm:gap-3">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ek-teal/15 text-ek-teal sm:h-10 sm:w-10">
              <Icon className="h-3.5 w-3.5 sm:h-[18px] sm:w-[18px]" aria-hidden />
            </span>
            <div className="min-w-0 pt-0.5">
              <p className="text-[7px] font-bold leading-snug tracking-[0.03em] text-white uppercase sm:text-[10px] sm:tracking-[0.06em]">
                {label}
              </p>
