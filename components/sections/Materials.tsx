...
export function Materials({ materials }: { materials: string[] }) {
  return (
    <section id="materials" className="section-block-light" aria-labelledby="materials-heading">
        ...
        <StaggerReveal className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 lg:gap-4">
          {materials.map((item) => (
            <StaggerItem key={item}>
              <div className="material-card group flex items-start gap-3 rounded-lg border border-ek-navy/8 bg-white px-4 py-3.5 text-sm text-ek-navy transition-all duration-300 hover:-translate-y-0.5 hover:border-ek-teal/22 hover:shadow-[0_12px_32px_-20px_rgba(10,10,10,0.2)]">
